...
/**
 * ============
 * Helpers (moved from original authOptions.ts)
 * ============
 */
...
/**
 * ============
 * Firebase Admin config + Adapter (Node-only)
 * ============
 */
...
function getFirebaseAdminConfig() {
  ...
  if (!projectId || !clientEmail || !privateKeyEnv) {
    console.error("Missing Firebase Admin credentials in auth-full-options.ts");
    return null;
  }
...
/**
 * ============
 * Full NextAuth options (Node-only)
 * ============
 */
export const authFullOptions: NextAuthConfig = {
  ...authConfig,

  // Node-only: Firestore adapter
  adapter: createFirestoreAdapter(),

  // Keep your session maxAge (auth.config.ts only sets jwt strategy)
  session: {
    ...authConfig.session,
    maxAge: 30 * 24 * 60 * 60
  },
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          // and Google provider users will typically already be verified.)
          if (!authUserRes.data.emailVerified) {
          ...
          // Preserve the verification error message for the UI
  ...
  callbacks: {
    // Keep Edge-safe authorized from authConfig (it always returns true),
    // but now we extend with jwt/session.
